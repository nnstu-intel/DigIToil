...
def draw_bounding_box_on_image_array(image,
  ...
  return2 = []
  image_pil = Image.fromarray(np.uint8(image)).convert('RGB')
  ret = draw_bounding_box_on_image(image_pil, ymin, xmin, ymax, xmax, color,
                             thickness, display_str_list,
                             use_normalized_coordinates)
  for i in range(len(ret)):
        return2.append(ret[i-1])
...
def draw_bounding_box_on_image(image,
  ...
  for display_str in display_str_list[::-1]:
    ...
    rezalt.append(round(left))
    rezalt.append(round(right))
    rezalt.append(round(top))
    rezalt.append(round(bottom))
    rezalt.append(color)
...
def draw_bounding_boxes_on_image_array(image,
  ...
  rezalter = []
  #print('stsrt')
  rezalter.append(draw_bounding_boxes_on_image(image_pil, boxes, color, thickness,
                               display_str_list_list))
  #print(rezalter)
  np.copyto(image, np.array(image_pil))
  return rezalter
...
def visualize_boxes_and_labels_on_image_array(
  ...
  for box, color in box_to_color_map.items():
    ...
    ret = draw_bounding_box_on_image_array(
        image,
        ymin,
        xmin,
        ymax,
